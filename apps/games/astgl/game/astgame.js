...
class AstGame
{
    ...
    draw()
    {
        ...
        canvas.style.left = GAZCanvas.offset.x;
        canvas.style.top = GAZCanvas.offset.y;
        canvas.style.position = "absolute";
        gl.canvas.width = GAZCanvas.currentScreenSize.w;
        gl.canvas.height = GAZCanvas.currentScreenSize.h;
        gl.viewport(GAZCanvas.offset.x/2,GAZCanvas.offset.y/2,GAZCanvas.targetSize.w,GAZCanvas.targetSize.h);

        gl.clearColor(0.1,0.1,0.1,1);
        gl.clear(gl.COLOR_BUFFER_BIT);// | gl.DEPTH_BUFFER_BIT);
        gl.disable(gl.CULL_FACE);
    
        //glDebugRenderer.OnStartDrawing();
        glDebugRenderer.vertexIndex=0;
    
         glDebugRenderer.addRect(0,0, 1600,900, new GLColour(0,0,0,1) );
    
        for(var x=0;x< 13;x++)
        {
            for(var y=0;y<9;y++)
            {
                //glDebugRenderer.AddRect( x*64,y*53, 32,26, [1,0,0,1]);
            }
        }
        ...
        var m = Matrix.CreateOrthoOffCenter(0,1600,900,0,1,32000);
        glDebugRenderer.Draw(gl,m,0,gl.TRIANGLES);
    }
